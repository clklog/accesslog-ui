<template>
    <div class="documentation-container" :style="( ByTime || ByCalendar || ByActive)  ? 'min-height : 103px;'  : 'min-height:60px;' ">
      <div class="checkContent" >
        <div style="display: flex; margin-top: 15px;box-sizing: border-box;" >
          <div v-if="ByTime" class="check_item public_border_color" style="margin-bottom: 12px;">
            <span>时间:</span>
            <el-radio-group
              v-model="timeFlag"
              class="checkBoxStyle"
          ...
          <el-date-picker
            v-if="ByCalendar"
            class="timnePickCSS"
            style="margin-left: 20px; width: 250px; height: 30px;margin-bottom: 12px;"
            v-model="currentTime"
            ref="tiemPick"
            type="daterange"
            range-separator="至"
          ...
          <div v-if="ByActive" style="margin-left: 20px; height: 30px;margin-bottom: 12px;">
            <el-radio-group
              size="mini"
              v-model="activeValue"
              style="font-size: 13px; height: 30px"
        ...
        <div class="channelSecond">
          ...
          <div style="display: flex; position: fixed; right: 30px">
            ...
            <div v-if="ByDownLoad && !dateFlag" class="btnEvent" @click="download">
              <i
  ...
  export default {
    ...
    mounted() {
      ...
      localStorage.setItem('timeFlag','day')
    },
    computed: {
      channel() {
      ...
      commonParams: {
        ...
        handler(newVal, oldVal) {
          if (JSON.stringify(newVal) !== JSON.stringify(oldVal)) {
            this.$nextTick(() => {
              if (this.ByActive) {
                newVal.timeType = this.activeValue;
              }
    ...
    methods: {
      ...
      checkDateEvnet(val) {
        localStorage.setItem('timeFlag',null)
        this.checked = false; //暂定清空日期
        this.contrastFlag = false;
        this.checkLabel = "对比时间段";
        this.contrastValue = "";
      ...
      download() {
        ...
        switch (path) {
          ...
          case "/visitorAnalysis/newoldVisitor": {
            ...
            this.commonData.cols = cols;
            delete this.commonData.contrastValue;
            this.commonData.visitorType = "";
            exportVisitorDetailApi(this.commonData).then((res) => {
              let name = this.sliceTypeFile(res);
              blobDownloads(res.data, name);
            });
            break;
