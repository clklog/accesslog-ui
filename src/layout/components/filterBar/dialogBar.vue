<template>
  <div class="documentation-container">
    <div class="checkContent">
      <div style="display: flex; padding: 10px">
        <div v-if="ByTime" class="check_item public_border_color">
          <span style="white-space: nowrap">时间:</span>
          <el-radio-group
            v-model="timeFlag"
            class="checkBoxStyle"
            @change="handleChange"
            style="white-space: nowrap"
          >
            <el-radio label="day">今日</el-radio>
            <el-radio label="previous">昨日</el-radio>
            <el-radio label="week">近7天</el-radio>
            <el-radio label="month">近30天</el-radio>
            <!-- <el-radio label="year"> 日期范围 </el-radio> -->
          </el-radio-group>
        </div>
        <!-- 日期1 -->
        <el-date-picker
          v-if="ByCalendar"
          class="timnePickCSS"
          style="margin-left: 20px; width: 250px; height: 30px"
          v-model="currentTime"
          ref="tiemPick"
          type="daterange"
          range-separator="至"
          start-placeholder="开始日期"
          end-placeholder="结束日期"
          value-format="yyyy-MM-dd"
          :picker-options="pickerBeginOption"
          @change="checkDateEvnet"
        >
        </el-date-picker>
        <!-- 地域调整到第二列 -->
        <div v-if="ByArea" class="areaContent">
          <div class="areaItem">
            <div class="areaHead public_border_color">地域</div>
            <div class="area_select sing_special_popover">
              <el-popover
                placement="bottom"
                width="510"
                trigger="click"
                v-model="popflag"
              >
                <div style="position: relative">
                  <el-radio-group size="mini" v-model="areaValue">
                    <el-radio
                      style="margin-top: 11px"
                      @change="handleCheckProvince(item)"
                      v-for="(item, index) in provinceData"
                      :key="index"
                      :label="item.provinceName"
                      >{{ item.provinceName }}</el-radio
                    >
                  </el-radio-group>
                  <div class="popCancle" @click="canclePopEvent">X</div>
                </div>
                <div class="areaBox public_font_color" slot="reference">
                  {{ areaValue }}
                  <i v-if="popflag" class="el-icon-arrow-up iconArrow"></i>
                  <i v-else class="el-icon-arrow-down iconArrow"></i>
                </div>
              </el-popover>
            </div>
          </div>
        </div>
        <!-- 对比时间段 -->
        <div style="margin-left: 20px; line-height: 30px" v-if="ByContrast">
          <el-checkbox v-model="checked" @change="checkTimeSlot">{{
            checkLabel
          }}</el-checkbox>
        </div>
        <div v-show="contrastFlag">
          <!-- 日期2 -->
          <el-date-picker
            v-if="ByCalendar"
            class="timnePickCSS"
            style="margin-left: 20px; width: 250px; height: 30px"
            v-model="contrastValue"
            type="daterange"
            range-separator="至"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
            value-format="yyyy-MM-dd"
            :picker-options="pickerBeginOptionTwo"
            @change="checkContrast"
          >
          </el-date-picker>
        </div>
        <div v-if="ByData" style="margin-left: 20px; height: 30px">
          <el-radio-group
            size="mini"
            v-model="timeType"
            style="font-size: 13px; height: 30px"
          >
            <el-radio-button
              v-if="Byhour"
              label="hour"
              :disabled="dayFlag ? true : false"
              >按时</el-radio-button
            >
            <el-radio-button label="day" :disabled="dayFlag ? true : false"
              >按日</el-radio-button
            >
            <el-radio-button label="week" :disabled="weekFlag ? true : false"
              >按周</el-radio-button
            >
            <el-radio-button label="month" :disabled="monthFlag ? true : false"
              >按月</el-radio-button
            >
          </el-radio-group>
        </div>
        <div v-if="ByActive" style="margin-left: 20px; height: 30px">
          <el-radio-group
            size="mini"
            v-model="activeValue"
            style="font-size: 13px; height: 30px"
            @change="handleDateFilter"
          >
            <el-radio-button
              v-for="item in active_options"
              :key="item.value"
              :value="item.value"
              :label="item.value"
              >{{ item.label }}</el-radio-button
            >
          </el-radio-group>
        </div>
        <!-- 选择时间范围 用户行为分析 -->
        <div v-if="ByTimeSlot" class="check_item public_border_color">
          <span>时间:</span>
          <el-radio-group
            v-model="timeSlotValue"
            class="checkBoxStyle"
            @change="handleMultipleDate"
          >
            <el-radio
              v-for="item in date_options"
              :key="item.value"
              :value="item.value"
              :label="item.value"
              >{{ item.label }}</el-radio
            >
          </el-radio-group>
        </div>
      </div>
      <div class="channelSecond" v-if="byChnnel">
        <div style="display: flex">
          <div class="check_item public_border_color">
            <span>渠道:</span>
            <el-radio-group
              v-model="channelValue"
              @change="channelEvent"
              style="margin-right: 10px"
            >
              <el-radio
                v-for="item in allChaneList"
                :label="item.id"
                :key="item.id"
                >{{ item.label }}</el-radio
              >
            </el-radio-group>
          </div>
          <div class="check_item public_border_color" v-if="ByVisitor">
            <span>访客:</span>
            <el-radio-group v-model="visitorType" class="checkBoxStyle">
              <el-radio label="">全部</el-radio>
              <el-radio label="新访客">新访客</el-radio>
              <el-radio label="老访客">老访客</el-radio>
            </el-radio-group>
          </div>

          <div class="check_item public_border_color" v-if="ByDimension">
            <span>分析维度:</span>
            <el-radio-group v-model="dimensionValue" class="checkBoxStyle">
              <el-radio label="out">流失</el-radio>
              <el-radio label="save">留存</el-radio>
            </el-radio-group>
          </div>
        </div>
        <div style="display: flex; position: absolute; right: 30px">
          <div
            v-if="ByDownLoad && !dateFlag"
            class="btnEvent"
            @click="downloadDialog"
          >
            <i
              class="el-icon-download"
              style="padding-right: 3px; font-size: 14px"
            ></i
            >下载
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { province } from "@/utils/province";
import { copyObj } from "@/utils/copy";
export default {
  props: {
    Byhour: {
      type: Boolean,
      default: true,
    },
    bySub: {
      type: Boolean,
      default: false,
    },
    byChnnel: {
      type: Boolean,
      default: false,
    },
    ByData: {
      type: Boolean,
      default: false,
    },
    ByArea: {
      type: Boolean,
      default: false,
    },
    // 新增事件
    ByContrast: {
      type: Boolean,
      default: false,
    },
    ByActive: {
      type: Boolean,
      default: false,
    },
    ByTimeSlot: {
      type: Boolean,
      default: false,
    },
    ByDimension: {
      type: Boolean,
      default: false,
    },

    // 默认展示
    ByTime: {
      type: Boolean,
      default: true,
    },
    ByCalendar: {
      type: Boolean,
      default: true,
    },
    ByVisitor: {
      type: Boolean,
      default: false,
    },
    ByDownLoad: {
      type: Boolean,
      default: false,
    },
  },
  data() {
    return {
      objParams: {},
      dateFlag: null, //触发的日历个数
      active_options: [
        {
          value: "day",
          label: "日活跃",
        },
        {
          value: "week",
          label: "周活跃",
        },
        {
          value: "month",
          label: "月活跃",
        },
      ],
      date_options: [],
      dateWeek_options: [
        {
          value: "3",
          label: "近4周",
        },
        {
          value: "7",
          label: "近8周",
        },
        {
          value: "11",
          label: "近12周",
        },
      ],

      checked: false,
      contrastFlag: false,
      checkLabel: "对比时间段",
      contrastValue: "", //对比时间
      contrastList: [],
      dimensionValue: "save",
      activeValue: "day",
      timeSlotValue: "day", //活跃用户默认为近7天
      pickerBeginOption: {
        disabledDate(date) {
          const today = new Date();
          return date.getTime() > today.getTime();
        },
        shortcuts: [
          {
            text: "近一周",
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 7);
              picker.$emit("pick", [start, end]);
            },
          },
          {
            text: "近一个月",
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 30);
              picker.$emit("pick", [start, end]);
            },
          },
          {
            text: "近三个月",
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 90);
              picker.$emit("pick", [start, end]);
            },
          },
          {
            text: "近六个月",
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 180);
              picker.$emit("pick", [start, end]);
            },
          },
          {
            text: "近一年",
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 365);
              picker.$emit("pick", [start, end]);
            },
          },
        ],
      },
      pickerBeginOptionTwo: {
        disabledDate(date) {
          const today = new Date();
          return date.getTime() > today.getTime();
        },
      },
      showBtn: true,
      dateRange: null, //日期范围
      isIndeterminate: false,
      checkAll: false,
      channelList: "",
      currentTime: "",
      allChaneList: [
        { label: "全部", id: "" },
        { label: "安卓", id: "安卓" },
        { label: "苹果", id: "苹果" },
        { label: "网站", id: "网站" },
        { label: "微信小程序", id: "微信小程序" },
      ],
      dateList: "day",
      visitorValue: "新访客",
      dayFlag: true,
      weekFlag: true,
      monthFlag: true,
      checkDateTime: "",
      past7daysStart: "", //最近一周
      timeDifference: "",

      // 接口参数数据
      // timeType: this.ByData ? "hour" : "day",
      timeType: this.Byhour ? "hour" : "day",
      timeFlag: "day",
      startTime: "",
      endTime: "",
      areaValue: "全部",
      channelValue: "", //渠道
      visitorType: "",
      popflag: false,
      commonData: "",
      openTimeFlag: false, //开启赋值
    };
  },
  created() {
    if (
      this.$route.name == "silentUsers" ||
      this.$route.name == "retainedUsers"
    ) {
      this.active_options = [
        {
          value: "day",
          label: "按日",
        },
        {
          value: "week",
          label: "按周",
        },
        {
          value: "month",
          label: "按月",
        },
      ];
    }
  },
  mounted() {
    this.initCheckDayNum();
  },
  computed: {
    channel() {
      if (this.channelValue) {
        return [this.channelValue];
      } else {
        return [];
      }
    },
    province() {
      if (this.areaValue == "全部") {
        return [];
      } else {
        return [this.areaValue];
      }
    },
    provinceData() {
      return province;
    },
    // 默认参数
    defaultParams() {
      const { startTime, endTime, channel } = this;
      return {
        startTime,
        endTime,
        channel,
      };
    },
    commonParams() {
      let obj = {};
      obj = Object.assign(obj, this.defaultParams);
      const { province, timeType, contrastValue, visitorType, dimensionValue } =
        this;
      if (this.ByArea) {
        obj = Object.assign(obj, { province });
      }
      if (this.ByData) {
        obj = Object.assign(obj, { timeType });
      }
      if (this.ByContrast) {
        obj = Object.assign(obj, { contrastValue });
      }
      if (this.ByVisitor) {
        obj = Object.assign(obj, { visitorType });
      }
      if (this.ByDimension) {
        // 流失留存
        obj = Object.assign(obj, { dimensionValue });
      }
      return obj;
    },
  },
  watch: {
    commonParams: {
      deep: false,
      immediate: false,
      handler(newVal, oldVal) {
        this.setTopFilterParams(newVal)
      },
    },
  },
  methods: {
    dialogbarEvent(commonParams) {
      this.timeFlag = localStorage.getItem("timeFlag");
      this.openTimeFlag = true;
      this.dialogParamsEvent(commonParams);
      if (
        commonParams.province instanceof Array &&
        commonParams.province.length > 0
      ) {
        this.areaValue = commonParams.province[0];
      }

      if (commonParams.channel instanceof Array) {
        if (commonParams.channel.length > 0) {
          this.channelValue = commonParams.channel[0];
        } else {
          this.channelValue = "";
        }
      } else {
        this.channelValue = commonParams.channel;
      }

      this.visitorType = commonParams.visitorType;
    },
    dialogParamsEvent(commonParams) {
      if (commonParams.timeFlag) {
        this.timeFlag = commonParams.timeFlag;
      }
      let val = this.timeFlag;
      let dateTime = this.toDateNow();
      let toData =
        new Date(new Date().toLocaleDateString()).getTime() + 8 * 3600 * 1000;
      if (val == "day") {
        this.currentTime = [dateTime, dateTime];
        this.dateTimeCount(1);
        if (this.Byhour) {
          this.timeType = "hour";
        } else {
          this.timeType = "day";
        }
        // this.timeType = "hour";
      } else if (val == "previous") {
        this.timeDifference = toData - 3600 * 24 * 1000;
        this.timestampToTime(this.timeDifference);
        this.currentTime = [this.checkDateTime, this.checkDateTime];
        this.dateTimeCount(1);
        // this.Byhour ? "hour" : "day",
        if (this.Byhour) {
          this.timeType = "hour";
        } else {
          this.timeType = "day";
        }
        // this.timeType = "hour";
      } else if (val == "week") {
        this.timeDifference = toData - 6 * 3600 * 24 * 1000;
        this.timestampToTime(this.timeDifference);
        this.currentTime = [this.checkDateTime, dateTime];
        this.dateTimeCount(6);

        this.timeType = "day";
      } else if (val == "month") {
        this.timeDifference = toData - 29 * 3600 * 24 * 1000;
        this.timestampToTime(this.timeDifference);
        this.currentTime = [this.checkDateTime, dateTime];
        this.dateTimeCount(29);
        this.timeType = "week";
      } else {
        this.currentTime = [commonParams.startTime, commonParams.endTime];
      }
      this.startTime = commonParams.startTime;
      this.endTime = commonParams.endTime;
    },
    // 日期初始值
    initDate(val) {
      if (this.ByActive) {
        const currentDate = new Date();
        let todateTime = this.$options.filters.formatDateToday();
        let sevenDaysAgo = new Date(currentDate);
        sevenDaysAgo.setDate(currentDate.getDate() - 6);
        let formattedSevenDaysAgo =
          this.$options.filters.formatDateYMD(sevenDaysAgo);
        formattedSevenDaysAgo;
        this.startTime = formattedSevenDaysAgo;
        this.endTime = todateTime;
      } else {
        // 默认页面调用当天日期
        let startTime = val[0].split("-").join("");
        let endTime = val[1].split("-").join("");
        this.startTime = val[0];
        this.endTime = val[1];
        let result = endTime - startTime;
        this.dateTimeCount(result);
      }
    },
    // 单选切换时间 timeType
    handleChange(val) {
      this.checked = false; //暂定清空日期
      this.contrastFlag = false;
      this.checkLabel = "对比时间段";
      this.contrastValue = "";

      if (this.checked) {
        this.dateFlag = 1;
      } else {
        this.dateFlag = null;
      }
      let dateTime = this.toDateNow();
      let toData =
        new Date(new Date().toLocaleDateString()).getTime() + 8 * 3600 * 1000;
      if (val == "day") {
        this.currentTime = [dateTime, dateTime];

        this.dateTimeCount(1);

        if (this.Byhour) {
          this.timeType = "hour";
        } else {
          this.timeType = "day";
        }
        // this.timeType = "hour";
      } else if (val == "previous") {
        this.timeDifference = toData - 3600 * 24 * 1000;
        this.timestampToTime(this.timeDifference);
        this.currentTime = [this.checkDateTime, this.checkDateTime];
        this.dateTimeCount(1);
        // this.Byhour ? "hour" : "day",
        if (this.Byhour) {
          this.timeType = "hour";
        } else {
          this.timeType = "day";
        }
        // this.timeType = "hour";
      } else if (val == "week") {
        this.timeDifference = toData - 6 * 3600 * 24 * 1000;
        this.timestampToTime(this.timeDifference);
        this.currentTime = [this.checkDateTime, dateTime];
        this.dateTimeCount(6);

        this.timeType = "day";
      } else {
        this.timeDifference = toData - 29 * 3600 * 24 * 1000;
        this.timestampToTime(this.timeDifference);
        this.currentTime = [this.checkDateTime, dateTime];
        this.dateTimeCount(29);
        this.timeType = "week";
      }

      this.startTime = this.currentTime[0];
      this.endTime = this.currentTime[1];
    },
    clearEvent() {
      this.areaValue = "全部";
    },
    channelEvent() {
      if (this.checked) {
        this.dateFlag = 3;
      } else {
        this.dateFlag = null;
      }
    },
    // 对比日期2
    checkContrast(val) {
      this.dateFlag = 2;
      let copyDate = copyObj(this.contrastValue);
      const dateOne =
        Date.parse(this.currentTime[1]) - Date.parse(this.currentTime[0]); // 日期一的时间差值
      const toDate = this.$options.filters.todateFunc(); //当日
      const rangeDate = Date.parse(toDate) - Date.parse(this.contrastValue[0]); //日期二的时间差值

      if (rangeDate >= dateOne) {
        const dateTwoEndtimeStamp = Date.parse(this.contrastValue[0]) + dateOne;
        let automaticDate =
          this.$options.filters.timeStampFunc(dateTwoEndtimeStamp);
        this.contrastValue[1] = automaticDate;
      } else {
        // 没有这个范围进行逆向取值
        const reverseDate = Date.parse(this.contrastValue[0]) - dateOne;
        let automaticDate = this.$options.filters.timeStampFunc(reverseDate);
        this.contrastValue[0] = automaticDate;
        this.contrastValue[1] = copyDate[1];
      }
    },

    // 按日按周筛选
    // 设置过去七天的日期
    handleDateFilter(val) {
      let toDay = this.toDateNow();
      switch (val) {
        case "day":
          if (this.$route.name == "retainedUsers") {
            // 不包含今天的日期
            // 昨日
            let toData =
              new Date(new Date().toLocaleDateString()).getTime() +
              8 * 3600 * 1000;
            this.timestampToTime(toData - 3600 * 24 * 1000);

            this.initCheckDayNum();
            this.timeSlotValue = "day";
            let sevenDaysAgo = new Date(new Date());
            sevenDaysAgo.setDate(new Date().getDate() - 7);
            let formattedSevenDaysAgo =
              this.$options.filters.formatDateYMD(sevenDaysAgo);
            this.startTime = formattedSevenDaysAgo;
            this.endTime = this.checkDateTime;
          } else {
            this.initCheckDayNum();
            this.timeSlotValue = "day";
            let sevenDaysAgo = new Date(new Date());
            sevenDaysAgo.setDate(new Date().getDate() - 6);
            let formattedSevenDaysAgo =
              this.$options.filters.formatDateYMD(sevenDaysAgo);
            this.startTime = formattedSevenDaysAgo;
            this.endTime = toDay;
          }

          break;
        case "week":
          if (this.$route.name == "retainedUsers") {
            this.date_options = [
              {
                value: "5",
                label: "近6周",
              },
              {
                value: "8",
                label: "近9周",
              },
              {
                value: "11",
                label: "近12周",
              },
            ];
            this.timeSlotValue = "5";
            this.checkWeekEvent(5);
          } else {
            this.date_options = this.dateWeek_options;
            this.timeSlotValue = "3";
            this.checkWeekEvent(3);
          }

          break;
        case "month":
          if (this.$route.name == "retainedUsers") {
            this.date_options = [
              {
                value: "5",
                label: "近6月",
              },
              {
                value: "8",
                label: "近9月",
              },
              {
                value: "11",
                label: "近12月",
              },
            ];
            this.timeSlotValue = "5";
            this.checkMonthEvent(5);
          } else {
            this.date_options = [
              {
                value: "2",
                label: "近3月",
              },
              {
                value: "5",
                label: "近6月",
              },
              {
                value: "11",
                label: "近12月",
              },
            ];
            this.timeSlotValue = "2";
            this.checkMonthEvent(2);
          }

          break;
      }
    },
    initCheckDayNum() {
      if (this.$route.name == "retainedUsers") {
        this.date_options = [
          {
            value: "day",
            label: "过去7天",
          },
          {
            value: "week",
            label: "过去14天",
          },
          {
            value: "month",
            label: "过去30天",
          },
        ];
      } else {
        this.date_options = [
          {
            value: "day",
            label: "近7天",
          },
          {
            value: "week",
            label: "近14天",
          },
          {
            value: "month",
            label: "近30天",
          },
        ];
      }
    },
    // 活跃日期

    handleMultipleDate(val) {
      if (this.activeValue == "day") {
        const currentDate = new Date();
        let todateTime = this.$options.filters.formatDateToday();
        switch (val) {
          case "day":
            let sevenDaysAgo = new Date(currentDate);
            sevenDaysAgo.setDate(currentDate.getDate() - 6);
            let formattedSevenDaysAgo =
              this.$options.filters.formatDateYMD(sevenDaysAgo);

            this.startTime = formattedSevenDaysAgo;
            this.endTime = todateTime;

            break;
          case "week":
            const fourteenDaysAgo = new Date(currentDate);
            fourteenDaysAgo.setDate(currentDate.getDate() - 13);
            const formattedFourteenDaysAgo =
              this.$options.filters.formatDateYMD(fourteenDaysAgo);
            this.startTime = formattedFourteenDaysAgo;
            this.endTime = todateTime;

            break;
          case "month":
            const thirtyDaysAgo = new Date(currentDate);
            thirtyDaysAgo.setDate(currentDate.getDate() - 29);
            const formattedThirtyDaysAgo =
              this.$options.filters.formatDateYMD(thirtyDaysAgo);
            this.startTime = formattedThirtyDaysAgo;
            this.endTime = todateTime;

            break;
        }
      } else if (this.activeValue == "week") {
        // 周活跃
        this.checkWeekEvent(val);
      } else if (this.activeValue == "month") {
        this.checkMonthEvent(val);
        // 月活跃
      }
    },
    checkMonthEvent(num) {
      let toDay = this.toDateNow();
      let deWeek = this.$options.filters.getFirstDayOfMonths(num);
      this.startTime = deWeek;
      this.endTime = toDay;
    },
    checkWeekEvent(num) {
      let toDay = this.toDateNow();
      let deWeek = this.$options.filters.getMondayOfPreviousWeeks(num);
      this.startTime = deWeek;
      this.endTime = toDay;
    },

    toDateNow() {
      var date = new Date();
      return (
        date.getFullYear() +
        "-" +
        (date.getMonth() + 1 < 10
          ? "0" + (date.getMonth() + 1)
          : date.getMonth() + 1) +
        "-" +
        (date.getDate() < 10 ? "0" + date.getDate() : date.getDate())
      );
    },

    checkDateEvnet(val) {
      this.checked = false; //暂定清空日期
      this.contrastFlag = false;
      this.checkLabel = "对比时间段";
      this.contrastValue = "";
      if (this.checked) {
        this.dateFlag = 1;
      } else {
        this.dateFlag = null;
      }
      this.timeFlag = "";
      this.startTime = val[0];
      this.endTime = val[1];
      let endTime = new Date(val[1]).getTime();
      let startTime = new Date(val[0]).getTime();
      let toDay =
        new Date(new Date().toLocaleDateString()).getTime() + 8 * 3600 * 1000;
      let timeStamp = endTime - startTime;

      if (timeStamp == 518400000) {
        this.dateList = "week";
      } else if (timeStamp == 2505600000) {
        this.dateList = "month";
      } else if (timeStamp == 86400000) {
        this.dateList = "previous";
      } else if (!timeStamp && startTime == toDay && endTime == toDay) {
        this.dateList = "day";
      } else {
        this.dateList = "";
      }
      let result = (endTime - startTime) / (3600 * 24 * 1000);
      this.dateTimeCount(result);

      // 选择日期进行按时按日自动刷选
      if (timeStamp == 0) {
        this.timeType = "hour";
      } else if (timeStamp >= 604800000 && timeStamp < 2592000000) {
        this.timeType = "day";
      } else if (timeStamp >= 2592000000 && timeStamp < 7776000000) {
        this.timeType = "week";
      } else {
        this.timeType = "month";
      }
    },
    checkTimeSlot(val) {
      switch (val) {
        case true:
          this.checkLabel = "对比";
          this.contrastValue = "";
          break;
        case false:
          this.checkLabel = "对比时间段";
          this.contrastValue = "";
          this.dateFlag = null;
          break;
      }
      this.contrastFlag = val;
    },
    btnShowEvent(val) {
      this.popflag = true;
    },
    canclePopEvent() {
      this.popflag = false;
    },
    downloadDialog() {
      this.$bus.$emit("publicDialogDown", this.commonData);
    },
    sliceTypeFile(res) {
      let fileName = res.headers["content-disposition"] || "";
      let index1 = fileName.indexOf("filename=");
      let result = fileName.substring(index1);
      result = decodeURIComponent(decodeURI(result.slice(9)));
      return result;
    },
    setTopFilterParams(val) {
      this.$emit("setFilterBarParams", val, this.dateFlag);
    },
    // 切换省份
    handleCheckProvince(e) {
      this.areaValue = e.provinceName;
    },

    // 日期计算
    dateTimeCount(result) {
      if (6 <= result < 29) {
        this.weekFlag = false;
        this.dayFlag = false;
        this.monthFlag = true;
        switch (this.timeType) {
          case "month":
            this.timeType = "week";
            break;

          default:
            break;
        }
      }
      if (result >= 29) {
        this.monthFlag = false;
        this.weekFlag = false;
        this.dayFlag = false;
      }
      if (result < 6) {
        this.dayFlag = false;
        this.monthFlag = true;
        this.weekFlag = true;
        switch (this.timeType) {
          case "month":
          case "week":
            this.timeType = "day";
            break;
        }
      }
    },

    // 默认初始日期
    handleAdd() {
      var date = new Date();
      let dateTime =
        date.getFullYear() +
        "-" +
        (date.getMonth() + 1 < 10
          ? "0" + (date.getMonth() + 1)
          : date.getMonth() + 1) +
        "-" +
        (date.getDate() < 10 ? "0" + date.getDate() : date.getDate());
      this.currentTime = [dateTime, dateTime];
      this.initDate(this.currentTime); //默认日期
    },

    // 时间戳转换器
    timestampToTime(timestamp) {
      var date = new Date(timestamp);
      var Y = date.getFullYear() + "-";
      var M =
        (date.getMonth() + 1 < 10
          ? "0" + (date.getMonth() + 1)
          : date.getMonth() + 1) + "-";
      var D =
        (date.getDate() < 10 ? "0" + date.getDate() : date.getDate()) + "";
      this.checkDateTime = Y + M + D;
    },

    handleCheckAllChange(val) {
      this.channelList = val ? this.allChaneList : [];
      this.isIndeterminate = false;
    },
    handleCheckedCitiesChange(value) {
      const checkedCount = value.length;
      if (checkedCount === this.allChaneList.length) {
        this.checkAll = false;
      } else {
        this.checkAll = false;
      }
      this.isIndeterminate =
        checkedCount > 0 && checkedCount < this.channelList.length;
    },
  },
};
</script>
<style lang="scss"></style>
<style lang="scss" scoped>
::v-deep {
  .el-radio-button--mini .el-radio-button__inner {
    padding: 9px 15px;
  }
  .el-checkbox__input.is-checked + .el-checkbox__label {
    color: #4d4d4d;
  }
  .el-checkbox__label {
    font-size: 13px;
  }
  @import "~@/styles/components/custom-radio.scss";

  // 日历样式start
  .el-range-editor--medium .el-range-input {
    font-size: 13px;
  }
  .el-date-editor .el-range__icon {
    font-size: 13px;
    line-height: 25px;
  }
  .el-range-editor--medium .el-range-separator {
    font-size: 13px;
    line-height: 25px;
  }
}
.popCancle:hover {
  border: 1px solid #727171;
  border-radius: 4px;
}
.popCancle {
  position: absolute;
  right: 0;
  top: 0;
  cursor: pointer;
  width: 20px;
  height: 20px;
  border-radius: 4px;
  border: 1px #d8e2ef solid;
  line-height: 20px;
  text-align: center;
}
.documentation-container {
  min-height: 50px;
  width: 100%;

  .checkContent {
    border-radius: 6px;
    width: 100%;
    min-height: 50px;
    z-index: 500;
    background-color: #fff;
    border-bottom: 1px #eee solid;

    //background-color: red;
    .areaContent {
      height: 3px;
      // margin-left: 15px;
      margin-left: 20px;
      .area_select {
        position: relative;
        .iconArrow {
          position: absolute;
          font-size: 13px;
          top: 3px;
          right: 5px;
          transform: translate(0, 50%);
        }
      }
      .areaItem {
        display: flex;
        align-items: center;
        .areaHead {
          width: 40px;
          height: 30px;
          line-height: 30px;
          text-align: center;
          font-size: 13px;
          font-weight: 400;
          color: #4d4d4d;
          border-top-left-radius: 5px;
          border-bottom-left-radius: 5px;
        }

        .areaBox {
          position: relative;
          height: 30px !important;
          line-height: 30px;
          font-size: 12px;
          font-weight: 400;
          border: 1px solid #d8e2ef;
          border-left: 0px;
          border-top-right-radius: 5px;
          border-bottom-right-radius: 5px;
          padding: 0 8px;
          padding-right: 20px;
          box-sizing: border-box;
          cursor: pointer;
        }
      }
    }
    .check_item {
      background-color: #ffffff;
      //margin-left: 20px;
      height: 30px;
      border-radius: 4px;
      display: flex;
      align-items: center;
      span {
        font-size: 13px;
        font-weight: 400;
        line-height: 13px;
        color: #4d4d4d;
        padding: 0 10px;
      }
      .checkBoxStyle {
        padding-right: 10px;
      }
    }
    .btnEvent {
      cursor: pointer;
      width: 65px;
      height: 28px;
      color: #194580;
      font-weight: 600px;
      border: 1px solid #d8e2ef;
      border-radius: 4px;
      font-size: 12px;
      line-height: 28px;
      text-align: center;
    }
    .SetSpace {
      margin-right: 9px;
    }
    .channelSecond {
      height: 57px;
      box-sizing: border-box;
      width: 100%;
      display: flex;
      padding: 12px 0 15px 0;
      align-items: center;
      position: relative;
    }
  }
}
</style>
