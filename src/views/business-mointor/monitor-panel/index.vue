...
export default {
  ...
  methods: {
    ...
    initEventApi() {
      this.toDate = this.commonParams.toDate
      let params = {};
      // console.log( this.commonParams,"4664456");
      if (this.commonParams.timeSlotList.length > 0) {
        params = {
          applicationCode: "cnb.mgt",
          summaryOption: "StatusLocation",
          startTime: this.commonParams.timeSlotList[0],
          endTime: this.commonParams.timeSlotList[1],
        };
      } else {
        params = {
          applicationCode: "cnb.mgt",
    ...
    getYesterday(params, flag) {
      ...
      if (this.commonParams.timeSlotList.length > 0) {
        outDate = {
          applicationCode: "cnb.mgt",
          summaryOption: "StatusLocation",
          startTime: this.commonParams.timeSlotList[2],
          endTime: this.commonParams.timeSlotList[3],
        };
      } else {
        outDate = {
        applicationCode: "cnb.mgt",
      ...
      getEventListApi(outDate).then((res) => {
        if (res.code == 200) {
          ...
          this.$nextTick(() => {
            ...
            this.$refs.registration.filterBusinessEvent(this.businessListData,this.toDate);
            this.$refs.register.filterBusinessEvent(this.businessListData,this.toDate);
            this.$refs.useVehicle.filterBusinessEvent(this.businessListData,this.toDate);
          });
        }
      });
    },
